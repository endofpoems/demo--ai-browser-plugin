...
class MindMapRenderer {
  ...
  _renderNodes(svg, nodes, layers, offsetX, offsetY, depth, parentPos) {
    ...
    const totalHeight = layer.length * (this.nodeHeight + this.vGap) - this.vGap;
    ...
    for (const node of nodes) {
      ...
      const y = startY + actualIdx * (this.nodeHeight + this.vGap);
  ...
  _drawLine(svg, x1, y1, x2, y2, depth) {
    ...
    svg.insertBefore(path, svg.firstChild); // 连线在节点下方
  }
